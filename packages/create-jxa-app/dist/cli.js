  ...
  Promise.all([getInstallPackage(version, originalDirectory), getTemplateInstallPackage(template, originalDirectory)]).then(([packageToInstall, templateToInstall]) => {
    ...
    Promise.all([getPackageInfo(packageToInstall), getPackageInfo(templateToInstall)]).then(([packageInfo, templateInfo]) => checkIfOnline(useYarn).then(isOnline => ({
      ...
      if (supportsTemplates) {
        ...
        console.log('');
        console.log(`The ${kleur__default['default'].cyan(packageInfo.name)} version you're using ${packageInfo.name === 'react-scripts' ? 'is not' : 'may not be'} compatible with the ${kleur__default['default'].cyan('--template')} option.`);
        console.log('');
      }
...
function getInstallPackage(version, originalDirectory) {
  ...
  const scriptsToWarn = [{
    ...
    message: kleur__default['default'].yellow(`The react-scripts-ts package is deprecated. TypeScript is now supported natively in Create React App. You can use the ${kleur__default['default'].green('--template typescript')} option instead when generating your app to include TypeScript support. Would you like to continue using react-scripts-ts?`)
  }];
