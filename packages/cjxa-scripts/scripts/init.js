...
function tryGitCommit(appPath) {
  try {
    ...
    child_process.execSync('git commit -m "Initialize project using Create React App"', {
      stdio: 'ignore'
    });
    return true;
  } catch (e) {
...
module.exports = function (appPath, appName, verbose, originalDirectory, templateName) {
  ...
  if (!templateName) {
    console.log('');
    console.error(`A template was not provided. This is likely because you're using an outdated version of ${chalk__default['default'].cyan('create-react-app')}.`);
    console.error(`Please note that global installs of ${chalk__default['default'].cyan('create-react-app')} are no longer supported.`);
    console.error(`You can fix this by running ${chalk__default['default'].cyan('npm uninstall -g create-react-app')} or ${chalk__default['default'].cyan('yarn global remove create-react-app')} before using ${chalk__default['default'].cyan('create-react-app')} again.`);
    return;
  }
